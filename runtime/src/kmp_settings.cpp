...
static const char *__kmp_parse_single_omp_schedule(const char *name,
  ...
  //--------------------LB4OMP_extensions--------------------------
    ...
    sched = kmp_sch_tap;
  else if (!__kmp_strcasecmp_with_sentinel("fac", ptr, *delim)) /* FAC */
    sched = kmp_sch_fac;
  else if (!__kmp_strcasecmp_with_sentinel("faca", ptr, *delim)) /* FACa */
    sched = kmp_sch_faca;
  else if (!__kmp_strcasecmp_with_sentinel("fac2", ptr, *delim)) /* FAC2 */
    sched = kmp_sch_fac2;
  else if (!__kmp_strcasecmp_with_sentinel("fac2a", ptr, *delim)) /* FAC2a */
    sched = kmp_sch_fac2a;
  else if (!__kmp_strcasecmp_with_sentinel("wf", ptr, *delim)) /* WF */
    sched = kmp_sch_wf;
  else if (!__kmp_strcasecmp_with_sentinel("bold", ptr, *delim)) /* BOLD */
    sched = kmp_sch_bold;
  else if (!__kmp_strcasecmp_with_sentinel("awf_b", ptr, *delim)) /* AWF-B */
    sched = kmp_sch_awf_b;
  else if (!__kmp_strcasecmp_with_sentinel("awf_c", ptr, *delim)) /* AWF-C */
    sched = kmp_sch_awf_c;
  else if (!__kmp_strcasecmp_with_sentinel("awf_d", ptr, *delim)) /* AWF-D */
    sched = kmp_sch_awf_d;
  else if (!__kmp_strcasecmp_with_sentinel("awf_e", ptr, *delim)) /* AWF-E */
    sched = kmp_sch_awf_e;
  else if (!__kmp_strcasecmp_with_sentinel("af", ptr, *delim)) /* AF */
    sched = kmp_sch_af;
  else if (!__kmp_strcasecmp_with_sentinel("af_a", ptr, *delim)) /* AF-A */
    sched = kmp_sch_af_a;
...
} // __kmp_stg_parse_env_min

static void __kmp_stg_print_env_min(kmp_str_buf_t *buffer, char const *name,
                                   void *data) {
...
static void __kmp_stg_parse_env_mu(char const *name, char const *value,
                                   void *data) {
  __kmp_stg_parse_int(name, value, 0, INT_MAX, &__kmp_env_mu);
} // __kmp_stg_parse_env_mu

static void __kmp_stg_print_env_mu(kmp_str_buf_t *buffer, char const *name,
                                   void *data) {
  __kmp_stg_print_int(buffer, name, __kmp_env_mu);
} // __kmp_stg_print_env_mu

static void __kmp_stg_parse_env_sigma(char const *name, char const *value,
                                      void *data) {
  double sigma = __kmp_convert_to_double(value);
  if (sigma >= 0) {
    __kmp_env_sigma = sigma;
  } else {
    KMP_WARNING(StgInvalidValue, name, value);
  }
} // __kmp_stg_parse_env_sigma

static void __kmp_stg_print_env_sigma(kmp_str_buf_t *buffer, char const *name,
                                      void *data) {
  __kmp_str_buf_print(buffer, "   %s=%8.6f\n", name, __kmp_env_sigma);
} // __kmp_stg_print_env_sigma

static void __kmp_stg_parse_env_overhead(char const *name, char const *value,
                                         void *data) {
  __kmp_stg_parse_int(name, value, 0, INT_MAX, &__kmp_env_overhead);
} // __kmp_stg_parse_env_overhead

static void __kmp_stg_print_env_overhead(kmp_str_buf_t *buffer,
                                         char const *name, void *data) {
  __kmp_stg_print_int(buffer, name, __kmp_env_overhead);
} // __kmp_stg_print_env_overhead

static void __kmp_stg_parse_env_cpu_speed(char const *name, char const *value,
                                          void *data) {
  __kmp_stg_parse_int(name, value, 0, INT_MAX, &__kmp_env_cpu_speed);
} // __kmp_stg_parse_env_cpu_speed

static void __kmp_stg_print_env_cpu_speed(kmp_str_buf_t *buffer,
                                          char const *name, void *data) {
  __kmp_stg_print_int(buffer, name, __kmp_env_cpu_speed);
} // __kmp_stg_print_env_cpu_speed

static void __kmp_stg_parse_env_alpha(char const *name, char const *value,
                                      void *data) {
  double alpha = __kmp_convert_to_double(value);
  if (alpha >= 0) {
    __kmp_env_alpha = alpha;
  } else {
    KMP_WARNING(StgInvalidValue, name, value);
  }
} // __kmp_stg_parse_env_alpha

static void __kmp_stg_print_env_alpha(kmp_str_buf_t *buffer, char const *name,
                                      void *data) {
  __kmp_str_buf_print(buffer, "   %s=%8.6f\n", name, __kmp_env_alpha);
} // __kmp_stg_print_env_alpha

static void __kmp_stg_parse_env_divider(char const *name, char const *value,
                                          void *data) {
  __kmp_stg_parse_int(name, value, 0, INT_MAX, &__kmp_env_divider);
} // __kmp_stg_parse_env_divider

static void __kmp_stg_print_env_divider(kmp_str_buf_t *buffer,
                                          char const *name, void *data) {
  __kmp_stg_print_int(buffer, name, __kmp_env_divider);
} // __kmp_stg_print_env_divider

static void __kmp_stg_parse_env_weights(char const *name, char const *value,
                                      void *data) {
  std::vector<double> weights = __kmp_convert_to_double_vector(value, ",");
  if (!weights.empty()) {
    __kmp_env_weights = weights;
  } else {
    KMP_WARNING(StgInvalidValue, name, value);
  }
} // __kmp_stg_parse_env_weights

static void __kmp_stg_print_env_weights(kmp_str_buf_t *buffer, char const *name,
                                      void *data) {
    std::stringstream ss;
    for(size_t i = 0; i < __kmp_env_weights.size(); ++i)
    {
        if(i != 0)
            ss << ",";
        ss << __kmp_env_weights[i];
    }
  __kmp_str_buf_print(buffer, "   %s\n", name, ss.str().c_str());
...
// OMP_PROC_BIND and proc-bind-var are supported, however.
   ...
   // {"KMP_MIN", __kmp_stg_parse_env_min, __kmp_stg_print_env_min, NULL, 0, 0},
    ...
    {"KMP_SIGMA", __kmp_stg_parse_env_sigma, __kmp_stg_print_env_sigma, NULL, 0,
     0},
    {"KMP_OVERHEAD", __kmp_stg_parse_env_overhead, __kmp_stg_print_env_overhead,
     NULL, 0, 0},
    {"KMP_CPU_SPEED", __kmp_stg_parse_env_cpu_speed,
     __kmp_stg_print_env_cpu_speed, NULL, 0, 0},
    {"KMP_ALPHA", __kmp_stg_parse_env_alpha, __kmp_stg_print_env_alpha, NULL, 0,
     0},
    {"KMP_DIVIDER", __kmp_stg_parse_env_divider, __kmp_stg_print_env_divider,
     NULL, 0, 0},
